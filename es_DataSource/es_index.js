...
/**
 * 函数名：start
 * 功能：es的客户端，用于向后台 es 服务端发送请求信息
 * 目标源：郭乃豪程序
 */
function start() {

    function connectServer() {
        var x = esSocket.connect(ipconfig.esPORT, ipconfig.esHOST);
    }
    ...
    esSocket.on('error', function (error) {
        if (flagConnect == 0) {
            commonSourceServer.errorLogFile.error('esSocket Error :' + error.toString());
            var connectError = {
                "TYPE":"602",
                "content" : "与elasticsearch后台通信失败！"
            }
            var ErrorArray = [];
    ...
    esSocket.on('close', function () {
        //var recentDate = new Date();
        console.log('esSocket connection closed on ' + recentDate);

    });

    esSocket.on('connect', function () {
        console.log('[esSocket] connect Ok.');
        flagConnect = 0;
        commonSourceServer.EventEmitter.on("sendESRequest", function () {
            if (!!commonSourceServer.esStrArray[0]) {
                ...
                if (RecentProcess) {
                     ...
                     * 将请求信息重新做包发送给后台
                     **/
                    var RequestStr = commonSourceServer.esStrArray.shift();
                    //console.log(recentDate+':'+"RequestStr :"+RequestStr);
                    var SN = getSN();
                    commonSourceServer.esRequestSN.push(SN);
    ...
    esSocket.on('data', function (data) {
        try {
            ...
            commonSourceServer.errorLogFile.error("es_index.js bufferData function err :" + err);
        }
    });
}
...
/**
 * 函数名：sendData
 * 功能 ：将前台请求信息传送给后台 es 服务端
 * 参数 ：
 *   RequestStr ：前台请求信息
 *   SN ：向后台发送请求包的 SN 标识，暂时没用
 */
function sendData(RequestStr, SN) {
    commonSourceServer.requestLogFile.info("[es_index sendData]client send Data to ES Server:" + RequestStr);
    var len = Buffer.byteLength(RequestStr);

    var sendEsBuffer = new Buffer(len + 8);
    //console.log("len of send data : " + len);

    //写入2个字节特征码
    sendEsBuffer.writeUInt16BE(65534, 0);//0xfffe

    //写入2个字节编号
    sendEsBuffer.writeUInt16BE(SN, 2);

    //写入4个字节表示本次包长
    sendEsBuffer.writeUInt32BE(len, 4);

    //写入数据
    try {
        sendEsBuffer.write(RequestStr, 8);
    } catch (err) {
        commonSourceServer.errorLogFile.error("es_index.js sendData function sendDbBuffer.write(RequestStr, 8) err :" + err);
    }
    esSocket.write(sendEsBuffer);
}
...
function dealReceiveDataSJ(dealDataBuffer) {
    ...
    commonSourceServer.responseLogFile.info("ES Server response data :" + receiveDataString);
    // String 转换成 JSON
    var receiveDataJSON;
    try {
        receiveDataJSON = JSON.parse(receiveDataString);
    } catch (err) {
        commonSourceServer.errorLogFile.error("es_index.js dealReceiveData function receiveDataJSON = JSON.parse(receiveDataString) err :" + err);
    }
    commonSourceServer.esReceiveStrArray.push(receiveDataJSON);
    RecentProcess = true;
    commonSourceServer.EventEmitter.emit("receiveESData");
